...
This page explains the mathematical foundations and implementation details of the Nearest Neighbors with Momentum (kNN+p) algorithm.

## Mathematical Foundation

### Distance Metric

The kNN+p algorithm uses a composite distance metric that combines spatial proximity with momentum alignment:
...
Due to the momentum condition, the kNN+p algorithm inevitably skips some tracers. To assign $\gamma$ values to these skipped particles, an **autoencoder neural network** can interpolate based on phase-space location:
