...
class TestKNNPResult:
    """Tests for KNNPResult and accessors."""
    ...
    def test_result_structure(self):
        ...
        result = nearest_neighbors_with_momentum(pos, vel, start_idx=0, lam=0.0)

        assert "ordered_indices" in result
        assert "skipped_indices" in result
        assert "position" in result
        assert "velocity" in result
        assert isinstance(result["ordered_indices"], tuple)
        assert isinstance(result["skipped_indices"], tuple)
...
class TestNearestNeighborsWithMomentum:
    ...
    def test_simple_line(self):
        ...
        assert result["ordered_indices"] == (0, 1, 2, 3, 4)
    ...
    def test_simple_line_reverse(self):
        ...
        result = nearest_neighbors_with_momentum(pos, vel, start_idx=2, lam=1.0)
    ...
    def test_high_lambda_prefers_momentum_direction(self):
        ...
        result = nearest_neighbors_with_momentum(pos, vel, start_idx=0, lam=10.0)
    ...
    def test_max_dist_termination(self):
        ...
        result = nearest_neighbors_with_momentum(
            pos, vel, start_idx=0, lam=0.0, max_dist=5.0
        )

        # Should stop before reaching point 2 (at x=10)
        assert 2 not in result["ordered_indices"]
    ...
    def test_max_dist_skipped_indices(self):
        ...
        all_indices = set(result["ordered_indices"]) | set(result["skipped_indices"])
    ...
    def test_n_max_limits_iterations(self):
        ...
        result = nearest_neighbors_with_momentum(
            pos, vel, start_idx=0, lam=0.0, n_max=3
        )
    ...
    def test_terminate_indices(self):
        ...
        # Should stop when reaching index 2
        assert result["ordered_indices"][-1] == 2
    ...
    def test_invalid_start_idx_raises(self):
        ...
        with pytest.raises(ValueError, match="out of bounds"):
            nearest_neighbors_with_momentum(pos, vel, start_idx=10)

        with pytest.raises(ValueError, match="out of bounds"):
            nearest_neighbors_with_momentum(pos, vel, start_idx=-1)
    ...
    def test_single_point(self):
        ...
        result = nearest_neighbors_with_momentum(pos, vel, start_idx=0)
    ...
    def test_3d_helix(self):
        ...
        assert len(result["ordered_indices"]) == 20
        assert set(result["ordered_indices"]) == set(range(20))
...
class TestAlgorithmIntegration:
    ...
    def test_curved_stream(self):
        ...
        result = nearest_neighbors_with_momentum(pos, vel, start_idx=0, lam=2.0)
    ...
    def test_noisy_stream(self):
        ...
        assert set(result["ordered_indices"]) == set(range(n_points))

        # The ordering should roughly follow the x-coordinate
        ordered_pos, _ = get_ordered_w(result)
    ...
    def test_bidirectional_stream(self):
        ...
        result_forward = nearest_neighbors_with_momentum(
            pos, vel_forward, start_idx=2, lam=5.0
        )
        # Start from middle with backward velocity
        result_backward = nearest_neighbors_with_momentum(
            pos, vel_backward, start_idx=2, lam=5.0
        )
    ...
    def test_multidimensional_keys(self):
        ...
        assert len(result["ordered_indices"]) == 3
        assert result["ordered_indices"] == (0, 1, 2)
